...
const toast = (props: Omit<ToastEvent, 'id'>) => {
  const id = Math.random().toString(36).substring(2, 9)
  const newToast: ToastEvent = { ...props, id, open: true }
  
  toasts = [...toasts, newToast].slice(-5) // limit to 5 toasts
...
function Toaster() {
  const [activeToasts, setActiveToasts] = React.useState<ToastEvent[]>([])

  React.useEffect(() => {
    const listener = (newToasts: ToastEvent[]) => {
  ...
  return (
    <ToastProvider>
      {activeToasts.map(({ id, title, description, action, variant, open, ...props }) => (
